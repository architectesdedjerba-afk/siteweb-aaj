...
export const MemberSpacePage = () => {
  ...
  const [rolesRoleFilter, setRolesRoleFilter] = useState<
    'all' | 'admin' | 'representative' | 'member'
  >('all');
  ...
  const handleUpdateRole = async (
    targetUser: any,
    newRole: 'admin' | 'representative' | 'member'
  ) => {
    if (!isAdmin) return;
    ...
    if (!window.confirm(`Changer le rôle de ${targetUser.displayName} en « ${newRole} » ?`)) return;
  ...
  const handleUpdateStatus = async (
    ...
    if (targetUser.status === newStatus) return;
    setRolesUpdatingUid(targetUser.uid);
    try {
